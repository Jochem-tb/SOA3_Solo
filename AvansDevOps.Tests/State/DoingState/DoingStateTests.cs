...
{
    [Fact]
    public void MoveToTodo_WhenCalled_ChangesStateAndNotifiesObserver()
    {
        // Arrange
        var item = new BacklogItem();
        item.State = new Domain.State.DoingState(item);
        ...
        observerMock.Verify(o => o.Update("Item moved back to Todo (rework)", null), Times.Once);
    }

    [Fact]
    public void MoveToDoing_WhenAlreadyDoing_KeepsStateAsDoing()
    {
        // Arrange
        var item = new BacklogItem();
        item.State = new Domain.State.DoingState(item);
        ...
        // Assert
        Assert.IsType<Domain.State.DoingState>(item.State);
    }

    [Fact]
    public void MoveToReadyForTesting_WhenCalled_ChangesStateAndNotifiesObserver()
    {
        // Arrange
        var item = new BacklogItem();
        item.State = new Domain.State.DoingState(item);
        ...
        observerMock.Verify(o => o.Update("Item moved to ReadyForTesting", null), Times.Once);
    }
