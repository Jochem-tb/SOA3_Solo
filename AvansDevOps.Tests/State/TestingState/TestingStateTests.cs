...
public class TestingStateTests
{
    ...
    public void MoveToTodo_WhenCalled_ThrowsInvalidOperationException()
    {
        ...
        item.State = new Domain.State.TestingState(item);
    ...
    public void MoveToDoing_WhenCalled_ChangesStateAndNotifiesObserver()
    {
        ...
        observerMock.Verify(o => o.Update("Item moved back to Doing (failed testing)", null), Times.Once);
    }
    ...
    [Fact]
    public void MoveToTesting_WhenAlreadyTesting_KeepsState()
    {
        // Arrange
        var item = new BacklogItem();
        item.State = new Domain.State.TestingState(item);
        ...
        // Assert
        Assert.IsType<Domain.State.TestingState>(item.State);
    }

    [Fact]
    public void MoveToTested_WhenCalled_ChangesStateToTested()
    {
        // Arrange
        var item = new BacklogItem();
        item.State = new Domain.State.TestingState(item);
        ...
        observerMock.Verify(o => o.Update("Item moved to Tested", null), Times.Once);
    }
